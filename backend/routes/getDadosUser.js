...
router.get("/", (req, res) => {
  ...
  connection.query(
    "SELECT vinculoAoIfes, cafeDaManha, almoco, lancheDaTarde, jantar, vegetariano FROM users WHERE id = ?",
    [id],
    (error, results) => {
      if (error) {
...
router.post("/", (req, res) => {
  ...
  connection.query(
    "UPDATE users SET VinculoAoIfes = ?, cafeDaManha = ?, almoco = ?, lancheDaTarde = ?, jantar = ?, vegetariano = ? WHERE id = ?",
    [vinculo, cafeDaManha, almoco, lancheDaTarde, jantar, vegetariano, id],
    (error, results) => {
      if (error) {
